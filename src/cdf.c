...
int main(void)
{
    ...
    int* hg_gaussian = hg_gaussian_mem + BASE;
    for (int i = -BASE; i <= BASE; ++i) {
        double x = (double)i / BASE;
    ...
    const float dp = .001f/BASE;
    for (float p = dp; p < 1.f; p += dp) {
        __int128_t i = floorf(BASE*probitf(p) + .5f);
        assert(i <= INT64_MAX);
        assert(i >= INT64_MIN);
        if (imaxabs((int64_t)i) > BASE)
            continue;
        hg_from_probit[i]++;
    }

    #ifdef TEST_PROBIT
    printf("%i, %i\n", -BASE-1, hg_gaussian[0]*hg_from_probit[0]); // scale generated graph
    for (int i = -BASE; i <= BASE; ++i)
        printf("%i, %i\n", i,
